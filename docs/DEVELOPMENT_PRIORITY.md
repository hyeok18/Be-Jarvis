...
| 9 | WU-08 | P0 | 식당 상세·한 탭 반응·개별 영상 근거 UI | A3 (A1 인계) | WU-06 | AC-01~05, AC-15, AC-17~18 | **진행 중** | 계약·충돌 점검 완료; 서버 상세 route와 안전한 반응 UI 구현 대기 |
...
현재 재개 대상은 **WU-08 — 식당 상세·한 탭 반응·개별 영상 근거 UI**다.

1. [`2026-08-25_WU-06_public-map-mock-ui.md`](./development-logs/2026-08-25_WU-06_public-map-mock-ui.md)의 공개 반응·개인 매칭 분리 계약과 WU-09 최신 일지를 먼저 읽는다.
2. 선택 식당의 상세 영역을 만들고 `좋아요`, `그냥 그래요`, `싫어요`를 한 번의 탭으로 선택하게 한다.
3. 방문 증명이 없으면 반응이 개인 취향에만 저장된다는 안내와 공개 반영 조건을 분리해 표시한다.
4. `confirmed` 상태이며 최신인 YouTube 방문 근거만 원본 링크·구독자 원값·API 기준 시각과 함께 표시한다.
5. WU-09 `/api/reactions` 연결 파일을 수정하기 전에 B1 활성 브랜치·열린 PR과 겹침을 다시 확인한다.
