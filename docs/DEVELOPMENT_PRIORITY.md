...
| 17 | WU-16 | P0 | 발표 스냅샷·백업 모드 | B2+A1 | WU-15 | AC-14, AC-28 | 진행 중 | `?snapshot=1` 명시 백업 모드와 `?snapshot=1&cycle=1` 30초 발표 순환 구현. 전체 test 146 passed/2 skipped, typecheck, lint, Webpack build, snapshot·cycle HTTP 200 통과. 390/1440px 눈검수와 3회 리허설은 재개 지점 |
...
현재 대상은 **WU-15 — 30곳 실제 수직 통합**이며, 상태는 **막힘**이다. 공개 데이터·API와 실제 DB 경계 검증은 완료됐지만, 기존 YouTube API 키를 폐기·교체하고 Preview Config를 안전하게 등록한 뒤 실제 공개 smoke와 통합 회귀를 재개해야 한다.
