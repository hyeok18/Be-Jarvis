...
## 7. 브랜치 통합 결정 (2026-08-26)

사용자 결정에 따라 WU-15 UI 통합의 단일 담당자는 **hyeok18**이다. 담당자는 PR #13 (`codex/ui-baseline` → `main`)의 최신 상태를 기준으로 통합과 재검증을 조율한다.

| 브랜치 | 결정 | 처리 규칙 |
|---|---|---|
| `codex/ui-baseline` | 활성 통합 기준선 | hyeok18가 PR #13을 관리한다. 병합 전 최신 `main`과 활성 브랜치 충돌을 다시 검사한다. |
| `codex/kakao-map-update` | 비활성·보관 | `main`과 merge-base가 없으므로 merge, cherry-pick, 전체 복사를 금지한다. 필요한 시각 요소만 기준선에서 수동으로 재구현하며, 원격 브랜치는 삭제 승인 전까지 보관한다. |
| `codex/mobile-map-prototype` | 비활성·대체됨 | 홈·상세 변경은 `codex/ui-baseline`에 대체되었다. 직접 병합하지 않으며, 누락 여부를 비교해 필요한 변경만 hyeok18가 선택적으로 반영한다. 원격 브랜치는 삭제 승인 전까지 보관한다. |

이 결정은 두 이전 브랜치를 활성 통합 후보에서 제외하는 것이며, 커밋 이력 삭제 권한을 부여하지 않는다. `scripts/check-push-safety.mjs`도 두 브랜치를 보관 상태로 제외한다. 새 변경은 `codex/ui-baseline` 또는 별도 후속 브랜치에서만 진행한다.
