...
7. 구현 전에 현재 정보로 계산한 예상 소요시간을 범위로 알린다. 외부 대기·충돌 조정처럼 변동 요인이 있으면 별도 시간으로 분리한다.
8. 같은 안내에서 사용자가 기다리는 동안 할 수 있는 준비, 알아야 할 위험, 팀원에게 공유할 수정 예정 공용 파일을 구체적으로 알린다.
9. 이번 세션의 작업 단위 ID, 범위, 수정 예상 파일, 완료 조건을 사용자에게 짧게 알린다.

예상 시간이 의미 있게 달라지면 완료 시점까지 기다리지 말고 원인과 새 예상 범위를 즉시 알린다.
...
7. 완료 게이트와 문서 동기화를 통과한 변경은 필요한 파일만 즉시 커밋한다.
8. 푸시 직전에 `git fetch --prune origin`과 `pnpm run check:push-safety`를 실행하고 GitHub의 열려 있는 PR·팀원이 알린 활성 브랜치와 수정 파일을 대조한다.
9. 현재 원격 브랜치가 로컬보다 앞서 있거나, `origin/main` 또는 다른 활성 브랜치가 같은 파일을 수정했다면 푸시하지 않는다. 사용자와 팀에 겹친 branch·파일을 알리고 통합 담당을 정한 뒤 다시 검증한다.
10. 안전 확인을 통과해도 force push는 사용하지 않고 일반 push만 실행한다.
11. 사용자가 여러 작업 단위를 계속 진행하라고 명시하지 않았다면 다음 단위 구현은 시작하지 않고 세션을 종료한다.

다음 세션은 최신 개발일지를 읽고 `다음 작업에서는 어떻게 해야 하는가`의 첫 번째 미완료 항목부터 시작한다. 이전 결정을 다시 추측하거나 완료된 작업을 처음부터 반복하지 않는다.

커밋은 로컬 복구 지점이므로 완료 즉시 만들 수 있지만, 푸시는 공유 원격 상태를 바꾸므로 위 충돌 점검이 끝나기 전에는 실행하지 않는다. 충돌 후보를 임의로 무시하거나 다른 팀원의 원격 commit을 덮어쓰지 않는다.

## 7. 제품 안전·품질 불변 조건

- PRD의 필수 범위가 선택 범위보다 항상 우선한다.
- 공개 제품에 별점, 평균평점, 종합점수, 보정점수, 리뷰 신뢰도 백분율을 다시 만들지 않는다.
- 공개 반응 입력은 `좋아요`, `그냥 그래요`, `싫어요` 셋뿐이며 하나의 숫자로 환산하지 않는다.
- 공개 반응 집계는 활성 `counted` 상태만 사용하고 증명 없는 반응은 개인 취향에만 사용한다.
- 실제 반응은 인증, 사용자·식당당 한 개 현재 반응, 유효한 방문 증명, rate limit 없이 공개 집계에 반영하지 않는다.
- 위험 신호 하나만으로 댓글알바·반응알바·봇이라고 단정하지 않고 `held` 상태로 격리한다.
- 반응 원본과 변경·moderation 감사 이벤트를 분리하고 실패한 집계는 마지막 정상 projection을 덮어쓰지 않는다.
- P0에는 공개 자유서술 리뷰·댓글을 추가하지 않는다.
- 위치 체크인은 실제 식사를 보장한다고 표현하지 않으며 원본 위도·경도와 전체 GPS 응답을 저장·로그하지 않는다.
- 영수증은 기본 관문으로 강제하지 않고 P1의 선택적 단계 상승 증명으로만 다룬다.
- YouTube 페이지 HTML, 영상, 자막, 댓글을 scraping·다운로드·재게시하지 않고 공식 Data API만 사용한다.
- 자동 영상-식당 후보는 관리자가 원본 링크를 확인하기 전에는 공개하지 않는다.
- YouTube 비인가 API 데이터는 30일 안에 새로고침하거나 삭제하고, stale·삭제·비공개 근거를 공개하지 않는다.
- 구독자 수는 최신 API 원값으로만 표시·정렬한다. 숨김을 0으로 처리하거나 자체 공신력 점수·식당 점수로 변환·합산하지 않는다.
- YouTube 출처, 원본 링크, 메타데이터 기준 시각을 사용자 화면에 표시한다.
- 개인화 매칭은 공개 반응 분포와 분리하고 공개 품질순위처럼 표현하지 않는다.
- 안 먹는 음식은 개인화 점수 감점이 아니라 명시적 제외를 기본으로 하며 사용자가 해제할 수 있어야 한다.
- P0의 취향과 공개 미반영 개인 반응은 브라우저 로컬에만 저장한다. 서버 동기화를 시작하면 사용자 소유 RLS·삭제·초기화·비공개 캐시 조건을 먼저 구현한다.
...
- 제품 범위·수용 기준·반응·매칭·출처 계약: `PRD.md`
...
- WU-20 완료 전에는 WU-03 schema를 시작하지 않고, WU-03 전에는 Production project에 DDL, seed, RLS 변경을 적용하지 않는다.
...
- 작업 시작 시 YouTube API Services Developer Policies, Channels resource, quota 문서의 현재 내용을 공식 출처로 확인한다.
- YouTube Applications와 Google Applications를 직접 scraping하거나 scraped data를 받지 않는다.
- allowlist 채널의 uploads playlist를 증분 조회하고 `channels.list`, `playlistItems.list`, `videos.list`의 필요한 필드만 저장한다.
- 비인가 공개 API 데이터는 30일을 넘기지 않고 새로고침 또는 삭제하며 삭제·비공개 영상은 공개 근거에서 비활성화한다.
- `subscriberCount`는 API 원값 그대로 저장·표시하고 `hiddenSubscriberCount`가 참이면 숫자를 비운다.
- 여러 채널 지표를 합산하거나 구독자 수로 자체 파생 공신력·신뢰도 점수를 만들지 않는다.
- 자동 장소 후보는 `candidate`로만 저장하고 관리자 확인 전 공개 DTO에 포함하지 않는다.
- API 키는 서버 전용이고 YouTube 원본 응답 전체, 영상·자막·댓글을 로그나 개발일지에 남기지 않는다.
